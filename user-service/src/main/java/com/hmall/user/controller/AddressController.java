...
public class AddressController {
    ...
    @ApiOperation("新增订单地址")
    @PostMapping
    public void saveDeliveryAddress(@RequestBody AddressDTO address){
        address.setUserId(UserContext.getUser());
        addressService.save(BeanUtils.copyBean(address,Address.class));
    }


    @ApiOperation("修改地址")
    @PutMapping
    public void updateDeliveryAddress(@RequestBody AddressDTO address){
        addressService.updateById(BeanUtils.copyBean(address, Address.class));
    }


    @ApiOperation("删除地址")
    @DeleteMapping("/{daId}")
    public void removeDeliveryAddress(@PathVariable("daId") Integer daId){
